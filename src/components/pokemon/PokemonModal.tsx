        ...
        <div className="pt-1 flex flex-col sm:flex-row gap-2.5">
          ...
          >
            {isSubmitting ? (
              <>
                ...
                <span>{isCaptured ? 'Updating Details...' : 'Capturing Pokémon (3s)...'}</span>
              </>
            ) : (
              <>
          ...
          {isCaptured && !isSubmitting && (
            <button
              ...
              className="py-3 px-4 rounded-xl font-bold text-sm border border-red-200 dark:border-red-900/60 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors"
            >
              Release Pokémon
            </button>
          )}
      ...
      <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl animate-in zoom-in-95 duration-200 text-center">
        ...
        {isLoading && (
          <div className="py-12 space-y-4 animate-pulse">
            ...
            <div className="w-40 h-40 bg-slate-200 dark:bg-slate-800 rounded-full mx-auto" />
        ...
        {pokemon && (
          <>
            {/* Captured badge top-left if captured */}
            <div className="flex items-center justify-between mb-1">
              <span className="font-mono text-sm font-bold text-slate-400 dark:text-slate-500">
                {formattedId}
              </span>
              {isCaptured && (
                <span className="inline-flex items-center space-x-1 px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-950/70 text-emerald-700 dark:text-emerald-400 border border-emerald-300 dark:border-emerald-800 text-xs font-bold mr-8">
            ...
            <div className="relative w-40 h-40 mx-auto my-2 flex items-center justify-center">
              ...
              <img
                src={artworkUrl}
                alt={pokemon.name}
                className="relative z-10 w-36 h-36 object-contain drop-shadow-md transition-transform hover:scale-105 duration-200"
                onError={(e) => {
            ...
            {/* Name */}
            <h2 className="text-2xl sm:text-3xl font-black capitalize tracking-tight text-slate-900 dark:text-white mt-1">
              {pokemon.name}
            </h2>
            ...
            {/* Details section */}
            <div className="bg-slate-50 dark:bg-slate-950/60 rounded-2xl p-4 border border-slate-200/80 dark:border-slate-800 text-left space-y-3.5">
                  ...
                  {pokemon.stats.map((s) => {
                    ...
                    return (
                      <div key={s.stat.name} className="flex items-center text-xs">
                        <span className="w-14 font-semibold text-slate-500 dark:text-slate-400">
                          {label}
                        </span>
                        <span className="w-7 font-mono font-bold text-slate-800 dark:text-slate-200 text-right mr-2">
                          {s.base_stat}
                        </span>
            ...
            {/* Dashed Separator */}
            <div className="border-t-2 border-dashed border-slate-200 dark:border-slate-800 my-5" />
